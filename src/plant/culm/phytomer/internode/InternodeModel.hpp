...
public:
  ...
  void step_state(double t) {
    ...
    } else if (_inter_phase == VEGETATIVE and _plant_state == plant::ELONG and _lig == t) {
      _inter_phase = REALIZATION;
    } else if (_inter_phase == REALIZATION and _inter_len >= _inter_predim) {
      _inter_phase = MATURITY;
    } else if (_inter_phase == REALIZATION and (_plant_phase == plant::NOGROWTH
                                                or _plant_phase == plant::NOGROWTH3
                                                or _plant_phase == plant::NOGROWTH4)) {
      _inter_phase = REALIZATION_NOGROWTH;
    } else if (_inter_phase == REALIZATION_NOGROWTH and (_plant_phase == plant::GROWTH or _plant_phase == plant::NEW_PHYTOMER3)) {
      _inter_phase = REALIZATION;
