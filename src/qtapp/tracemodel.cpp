...
bool VisibleTraceModel::filterAcceptsRow(int sourceRow,const QModelIndex &sourceParent) const {
    ...
    bool accepted = true;
    if(!date.isEmpty()) accepted &= (date == index.data(DATE_ROLE).toString());
...
QVariant TraceModel::data(const QModelIndex &index, int role) const {
    if(role == Qt::DisplayRole) {
        ...
        return QString::fromStdString(artis::utils::DateTime::toJulianDayFmt(elements[index.row()].get_time(),artis::utils::DATE_FORMAT_YMD));
